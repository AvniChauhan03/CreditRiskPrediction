...
# Load the encoder (or if used, scaler)
...
def preprocess_input(input_data):
    # Map categorical values (same as you did while training)
    ...
    # Ensure the correct order of columns as per the model training
    columns = ['Age', 'Sex', 'Job', 'Housing', 'Saving accounts', 'Checking account', 'Credit amount', 'Duration', 'Purpose']
    input_data = input_data[columns]
    
    # If you need to apply one-hot encoding or any transformation like it was done while training, apply it here:
...
if st.button('Predict Credit Risk'):
    ...
    # Ensure input has the same number of features as the model expects
